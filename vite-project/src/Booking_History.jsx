            ...
            <div className="booking-info">
                ...
                <div className="show-details">
                    ...
                    <div className="show-time">
                        <div>{formatDate(booking?.bookingdate?.split("T")[0])}</div>
                        <GoDotFill />
                        <div>{t(booking?.bookingdate)}</div>
                    </div>
                </div>
                <div className="ticket-details">
                    <div><strong>Total Tickets:</strong> {booking?.seats?.length}</div>
                    <div><strong>Seat Number:</strong>
                        {booking?.seats?.map((item, index) => (
                            <span key={index}>
                                {item}{index !== booking?.seats?.length - 1 ? ', ' : ''}
                            </span>
                        ))}
                    </div>
                </div>
