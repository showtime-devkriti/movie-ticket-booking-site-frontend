import React from "react";
import { useState, useEffect, useRef } from "react";
import "./Sidebar.css";
import { TbLogout } from "react-icons/tb";
import { Link, useNavigate } from "react-router-dom";
import Cookies from "js-cookie";
import { IoMdClose } from "react-icons/io";

const CustomScrollbar = ({ children, height }) => {
    const contentRef = useRef(null);
    const thumbRef = useRef(null);

    const [thumbHeight, setThumbHeight] = useState(0);
    const [thumbTop, setThumbTop] = useState(0);
    const [dragging, setDragging] = useState(false);
    const [dragStartY, setDragStartY] = useState(0);
    const [initialThumbTop, setInitialThumbTop] = useState(0);

    // Calculate and set thumb height based on content
    useEffect(() => {
        const content = contentRef.current;
        if (!content) return;
        const visibleRatio = content.clientHeight / content.scrollHeight;
        const newThumbHeight = Math.max(visibleRatio * content.clientHeight, 20);
        setThumbHeight(newThumbHeight);
    }, [children, height]);

    // Update thumb position on scroll
    const handleScroll = () => {
        const content = contentRef.current;
        const scrollRatio = content.scrollTop / (content.scrollHeight - content.clientHeight);
        const maxThumbTop = content.clientHeight - thumbHeight;
        setThumbTop(scrollRatio * maxThumbTop);
    };

    // Start dragging
    const startDrag = (e) => {
        setDragging(true);
        setDragStartY(e.clientY);
        setInitialThumbTop(thumbTop);
        e.preventDefault();
    };

    // Drag thumb
    const onDrag = (e) => {
        if (!dragging) return;
        const content = contentRef.current;
        const deltaY = e.clientY - dragStartY;
        const maxThumbTop = content.clientHeight - thumbHeight;
        let newTop = initialThumbTop + deltaY;
        if (newTop < 0) newTop = 0;
        if (newTop > maxThumbTop) newTop = maxThumbTop;
        setThumbTop(newTop);

        // Scroll content accordingly
        const scrollRatio = newTop / maxThumbTop;
        content.scrollTop = scrollRatio * (content.scrollHeight - content.clientHeight);
    };

    // Stop dragging
    const stopDrag = () => {
        setDragging(false);
    };

    useEffect(() => {
        // Attach mousemove and mouseup globally while dragging
        if (dragging) {
            window.addEventListener("mousemove", onDrag);
            window.addEventListener("mouseup", stopDrag);
        } else {
            window.removeEventListener("mousemove", onDrag);
            window.removeEventListener("mouseup", stopDrag);
        }

        return () => {
            window.removeEventListener("mousemove", onDrag);
            window.removeEventListener("mouseup", stopDrag);
        };
    }, [dragging]);

    return (
        <div style={{ position: "relative", height, overflow: "hidden", paddingRight: "5px" }}>
            <div
                ref={contentRef}
                onScroll={handleScroll}
                style={{ overflowY: "scroll", height, paddingRight: "15px" /* prevent content hidden under scrollbar */ }}
            >
                {children}
            </div>

            {/* Scrollbar track */}
            <div
                style={{
                    position: "absolute",
                    top: 0,
                    right: 0,
                    width: 12,
                    height: "100%",
                    backgroundColor: "rgba(50,50,50)",
                    borderRadius: 6,
                }}
            >
                {/* Scrollbar thumb */}
                <div
                    ref={thumbRef}
                    onMouseDown={startDrag}
                    style={{
                        height: thumbHeight,
                        width: "100%",
                        backgroundColor: "white",
                        borderRadius: 6,
                        cursor: "grab",
                        position: "relative",
                        top: thumbTop,
                        userSelect: "none",
                    }}
                />
            </div>
        </div>
    );
};

const Sidebar = ({ isSidebar, setIsSidebar }) => {
    const navigate = useNavigate()

    const logout = () => {
        Cookies.remove("admin")
        navigate("/admin/login")
    }

    const [isMobile, setIsMobile] = useState(window.innerWidth <= 768);

    useEffect(() => {
        const handleResize = () => {
            setIsMobile(window.innerWidth <= 768);
        };

        window.addEventListener("resize", handleResize);
        return () => window.removeEventListener("resize", handleResize);
    }, []);

    return (
        <>
            <div className="sidebar-wrapper">
                <div className={isSidebar ? ("admin-sidebar open") : ("admin-sidebar close")}>
                    <div className="top">
                        {isMobile ? (
                            <div className="top-close">
                                <div className="admin-sidebar-logo">
                                    <Link to="/admin">
                                        <svg width="250" height="60" viewBox="0 0 500 200" xmlns="http://www.w3.org/2000/svg">
                                            <g transform="translate(-225,80) rotate(-60 100 100) scale(1.4,1.4)" >
                                                <path d="M80,40 
             a10,10 0 0,1 10,-10 
             h100 
             a10,10 0 0,1 10,10 
             v20 
             a10,10 0 0,0 0,20 
             v20 
             a10,10 0 0,1 -10,10 
             h-100 
             a10,10 0 0,1 -10,-10 
             v-20 
             a10,10 0 0,0 0,-20 
             z"
                                                    fill="#2479db" />
                                            </g>

                                            <g transform="translate(-115,65) scale(1.6,1.4)">
                                                <rect x="0" y="0" width="50" height="50" rx="6" fill="#111" />
                                                <polygon points="18,12 35,25 18,38" fill="#fff" />
                                                <g transform="rotate(-15 25 0)">
                                                    <rect x="0" y="-20" width="50" height="14" fill="#111" />
                                                    <rect x="4" y="-18" width="10" height="10" fill="#fff" />
                                                    <rect x="20" y="-18" width="10" height="10" fill="#fff" />
                                                    <rect x="36" y="-18" width="10" height="10" fill="#fff" />
                                                </g>
                                            </g>

                                            <text x="180" y="130" fontFamily="Sans" fontSize="99" fill="white" transform="translate(-150,0)" fontWeight="bold">
                                                SHOWTIME
                                            </text>
                                        </svg>
                                    </Link>
                                </div>
                                <button onClick={() => { setIsSidebar(!isSidebar) }}><IoMdClose size={55} /></button>
                            </div>
                        ) : (
                            <div className="admin-sidebar-logo">
                                <Link to="/admin">
                                    <svg width="250" height="60" viewBox="0 0 500 200" xmlns="http://www.w3.org/2000/svg">
                                        <g transform="translate(-225,80) rotate(-60 100 100) scale(1.4,1.4)" >
                                            <path d="M80,40 
             a10,10 0 0,1 10,-10 
             h100 
             a10,10 0 0,1 10,10 
             v20 
             a10,10 0 0,0 0,20 
             v20 
             a10,10 0 0,1 -10,10 
             h-100 
             a10,10 0 0,1 -10,-10 
             v-20 
             a10,10 0 0,0 0,-20 
             z"
                                                fill="#2479db" />
                                        </g>

                                        <g transform="translate(-115,65) scale(1.6,1.4)">
                                            <rect x="0" y="0" width="50" height="50" rx="6" fill="#111" />
                                            <polygon points="18,12 35,25 18,38" fill="#fff" />
                                            <g transform="rotate(-15 25 0)">
                                                <rect x="0" y="-20" width="50" height="14" fill="#111" />
                                                <rect x="4" y="-18" width="10" height="10" fill="#fff" />
                                                <rect x="20" y="-18" width="10" height="10" fill="#fff" />
                                                <rect x="36" y="-18" width="10" height="10" fill="#fff" />
                                            </g>
                                        </g>

                                        <text x="180" y="130" fontFamily="Sans" fontSize="99" fill="white" transform="translate(-150,0)" fontWeight="bold">
                                            SHOWTIME
                                        </text>
                                    </svg>
                                </Link>
                            </div>
                        )}
                        <div className="admin-info">
                            <div><img src="data:image/png;base64,"></img></div>
                            <div className="welcome">
                                <h1>Welcome</h1>
                                <h2>Manisharan73</h2>
                            </div>
                        </div>
                    </div>
                    <CustomScrollbar height="100%">
                        <div className="functions">
                            <Link to="/admin/profile"><div><h2>Admin Profile</h2></div></Link>
                            <Link to="/admin"><div><h2>Dashboard</h2></div></Link>
                            <Link to="/admin/shows"><div><h2>Add Shows</h2></div></Link>
                            <Link to="/admin/screens"><div><h2>Add Screens</h2></div></Link>
                        </div>
                    </CustomScrollbar>
                    <div className="admin-logout" onClick={logout}>
                        <h2>Logout </h2>
                        <TbLogout size={30} />
                    </div>
                </div>
            </div>
        </>
    );
};

export default Sidebar;